...
void LCD_ShowChinese(unsigned int x,unsigned int y,unsigned char len,unsigned char *index,unsigned int fc,unsigned int bc,unsigned char sizey)
{
	...
	for(i=0;i<len;i++)
	{
		...
		else if(sizey==40) LCD_ShowChinese40x40(x+i*sizey,y,fc,bc,index[i]);
		else LCD_ShowChinese40x40(x+i*sizey,y,fc,bc,index[i]);
	}
}
/******************************************************************************
      函数说明：显示单个16x16汉字
...
/******************************************************************************
      ...
      返回值：  无
******************************************************************************/
void LCD_ShowChinese40x40(unsigned int x,unsigned int y,unsigned int fc,unsigned int bc,unsigned char index)
{
	unsigned int i,j;
	LCD_Address_Set(x,y,x+39,y+39);
	for(i=0;i<200;i++)
	{
		for(j=0;j<8;j++)
		{	
			if(tfont40[index][i]&(0x01<<j))LCD_WR_DATA(fc);
					else LCD_WR_DATA(bc);
		}
	}
} 
...
******************************************************************************/
void DisplayScreen(unsigned char *dataEPC,unsigned char *dataV,unsigned char *dataA,unsigned char *dataP,unsigned char *dataCOS,unsigned char *dataHZ,unsigned char *dataKWH)
{
  	static unsigned char chuzhi[2];
  	LCD_ShowPicture(0,0,320,240,gImage_1);
  	chuzhi[0]=0;chuzhi[1]=0;
  	LCD_ShowChinese(10,190,2,chuzhi,BLUE,LIGHTBLUE,43);
  	LCD_ShowChinese(170,190,2,chuzhi,BLUE,LIGHTGREEN,43);
}